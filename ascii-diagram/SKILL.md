---
name: ascii-diagram
description: 绘制 ASCII/Unicode 纯文本图：系统架构、数据流、状态机、表格与简易线框图。优先中心轴对齐、几何对称、等宽边框和最小信息集。
---

# ASCII Diagram Skill

## 目标
用最小篇幅画出高可扫描的 plaintext 图。

默认顺序是：**先定几何布局，再填语义标签，最后压缩文本**。

## 何时使用
- 用户要 ASCII 图、架构图、流程图、状态机、表格或 wireframe
- 用户强调“对齐 / 对称 / 居中 / 等宽 / 版式优化”
- 内容需要直接贴进 Markdown、终端、注释或文档

## 核心原则
1. **一图一事**：一个图只表达一个主视角。
2. **当前有效**：优先画 as-is，不混入未来方案。
3. **几何优先**：先保证对齐、对称、间距、边框一致，再谈修饰。
4. **节点有语义**：每个节点都代表角色、状态、数据或容器。
5. **连线可读**：方向明确，必要时加短标签，如 `HTTP`、`Event`、`Batch`。
6. **局部先汇聚**：多分支优先在局部合流，再汇入主干。
7. **最小集合**：能删的节点、边、文字就删。
